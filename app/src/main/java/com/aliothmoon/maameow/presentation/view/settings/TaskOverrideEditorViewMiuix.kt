                ...
                actions = {
                    IconButton(
                        ...
                        Icon(
                            ...
                            tint = if (isJsonValid) colorScheme.primary
        ...
        Row(
            ...
            IconButton(onClick = { editor?.undo() }, enabled = canUndo) {
                Icon(
                    ...
                    tint = if (canUndo) colorScheme.onSurfaceVariant
                    else colorScheme.onSurface.copy(alpha = 0.38f)
                )
            }
            IconButton(onClick = { editor?.redo() }, enabled = canRedo) {
                Icon(
                    ...
                    tint = if (canRedo) colorScheme.onSurfaceVariant
                    else colorScheme.onSurface.copy(alpha = 0.38f)
                )
            }
...
private fun JsonStatusBar(isValid: Boolean) {
    ...
    Surface(color = bg) {
        Row(
            ...
            Text(text = label, style = MiuixTheme.typography.labelMedium, color = fg)
        }
    }
}
