...
import com.aliothmoon.maameow.ui.screen.panel.TaskConfigPanel
import com.aliothmoon.maameow.ui.screen.panel.TaskListPanel
import com.aliothmoon.maameow.ui.screen.panel.AutoBattlePanel
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            modifier = Modifier
                ...
                .padding(top = 8.dp, bottom = LocalFloatingBottomBarHeight.current + 8.dp)
        ) {
            // --- 预览图区域：实时加载 ---
            Box(
                modifier = Modifier
                            ...
                            Row(
                                ...
                                MiuixButton(
                                    ...
                                    if (maaState == MaaExecutionState.STARTING) {
                                        ...
                                        MiuixText(stringResource(R.string.task_btn_start))
                                    }
                                }
        ...
        BackgroundMoreActionsDialog(
            show = showMoreActions,
            isGameMuted = viewModel.isGameMuted.collectAsStateWithLifecycle().value,
        ...
        activeDialog?.let { dialog ->
            ...
            val confirmColor = when (dialog.type) {
                ...
                PanelDialogType.WARNING -> MiuixTheme.colorScheme.onSecondaryVariant
...
@Composable
private fun BackgroundMoreActionsDialog(
    show: Boolean,
    isGameMuted: Boolean,
    ...
    // Miuix DialogLayout provides window-dim + back-handler + dismiss lifecycle.
    // enableAutoLargeScreen=false keeps the small-screen layout (no bottom-sheet
    // snap) and lets us force Center alignment below.
    ...
    DialogLayout(
        ...
        // Click-outside-to-dismiss on the dim layer; the card below swallows
        // the click so taps on the card content do not dismiss.
        Box(
            modifier = Modifier
                .fillMaxSize()
        ...
        if (showHardwareScreenOffConfirm) {
            AdaptiveTaskPromptDialog(
                ...
                confirmColor = MiuixTheme.colorScheme.primary,
            )
        }
    }
}
