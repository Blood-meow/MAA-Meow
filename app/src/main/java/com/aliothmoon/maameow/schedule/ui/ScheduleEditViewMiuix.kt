...
fun ScheduleEditViewMiuix(
        ...
        LazyColumn(modifier = Modifier.fillMaxWidth().padding(padding)) {
                ...
                SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp)) {
                    ScheduleType.entries.forEachIndexed { index, type ->
                        SegmentedButton(
                            ...
                            shape = SegmentedButtonDefaults.itemShape(index = index, count = ScheduleType.entries.size, baseShape = RoundedCornerShape(4.dp))
                        ) {
                            Text(when (type) {
                    ...
                    item {
                        FlowRow(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            DayOfWeek.entries.forEach { day ->
                                FilterChip(selected = day in state.daysOfWeek, onClick = { viewModel.onToggleDay(day) }, label = { Text(scheduleDayChipLabel(day)) }, colors = chipColors)
                            }
                        }
                    }
                    ...
                    item {
                        ...
                        FlowRow(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            state.executionTimes.forEach { time ->
                                InputChip(selected = false, onClick = { editingTime = time; showTimePicker = true },
                                    label = { Text(time.format(timeFormatter)) },
                                    trailingIcon = {
                                        IconButton(onClick = { viewModel.onRemoveTime(time) }, modifier = Modifier.size(18.dp)) {
                                            Icon(Icons.Default.Close, contentDescription = stringResource(R.string.common_delete), modifier = Modifier.size(14.dp))
                                        }
                                    }
                                )
                            }
                            AssistChip(onClick = { editingTime = null; showTimePicker = true },
                ...
                ScheduleType.INTERVAL -> {
                    ...
                    item {
                            ...
                            interactionSource = remember { androidx.compose.foundation.interaction.MutableInteractionSource() }.also { source ->
                                LaunchedEffect(source) {
                                    source.interactions.collect { interaction ->
                                        if (interaction is androidx.compose.foundation.interaction.PressInteraction.Release) showDatePicker = true
                                    }
                                }
                            }
                        )
                        ...
                        if (showDatePicker) {
                            ...
                            DatePickerDialog(
                                ...
                                confirmButton = { TextButton(onClick = { pendingDateMs = datePickerState.selectedDateMillis; showDatePicker = false; showStartTimePicker = true }) { Text(stringResource(R.string.schedule_next_step)) } },
                    ...
                    item {
                        Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                            ...
                            if (totalMinutes > 0) {
                                MiuixText(text = stringResource(R.string.schedule_total_hours, totalMinutes / 60), style = MaterialTheme.typography.bodySmall, color = MiuixTheme.colorScheme.onSurfaceSecondary)
                            }
                        }
                    }
                }
            ...
            item {
                if (state.profiles.isEmpty()) {
                    MiuixText(text = stringResource(R.string.schedule_no_profiles), style = MaterialTheme.typography.bodyMedium, color = MiuixTheme.colorScheme.onSurfaceSecondary, modifier = Modifier.padding(horizontal = 16.dp))
                } else {
                    FlowRow(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    if (!enabledTasks.isNullOrEmpty()) {
                        MiuixText(text = stringResource(R.string.schedule_enabled_tasks, enabledTasks), style = MaterialTheme.typography.bodySmall, color = MiuixTheme.colorScheme.onSurfaceSecondary, modifier = Modifier.padding(start = 16.dp, end = 16.dp, top = 8.dp))
                    }
                }
            }
    ...
    if (showTimePicker) {
        TimePickerDialog(initialTime = editingTime, onDismiss = { showTimePicker = false }, onConfirm = { time ->
            val old = editingTime
            if (old != null) viewModel.onReplaceTime(old, time) else viewModel.onAddTime(time)
    ...
    BasicAlertDialog(onDismissRequest = onDismiss) {
        MiuixSurface(shape = MaterialTheme.shapes.extraLarge, color = MiuixTheme.colorScheme.surface) {
            Column(modifier = Modifier.padding(24.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                MiuixText(text = stringResource(R.string.schedule_time_picker_title), style = MaterialTheme.typography.labelMedium, color = MiuixTheme.colorScheme.onSurfaceSecondary, modifier = Modifier.fillMaxWidth().padding(bottom = 20.dp))
